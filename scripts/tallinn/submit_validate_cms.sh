...
# END=`wc -l data/cms/run3/tallinn/jetmet0.txt | cut -f1 -d' '`
# for ifile in $(seq 1 $END); do
#     sbatch $SUBSCRIPT scripts/cmssw/validation_job_data.sh False mlpf data/cms/run3/tallinn/jetmet0.txt JetMET0 $ifile $SITE
#     sbatch $SUBSCRIPT scripts/cmssw/validation_job_data.sh False pf data/cms/run3/tallinn/jetmet0.txt JetMET0 $ifile $SITE
# done
#
