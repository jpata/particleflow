...
from Validation.RecoTrack.plotting.plotting import (
    Plot,
    PlotGroup,
    PlotFolder,
    Plotter,
...
from Validation.RecoParticleFlow.defaults_cfi import (
    ptbins,
    etabins,
    response_distribution_name,
    muLowOffset,
    muHighOffset,
    npvLowOffset,
    npvHighOffset,
...
def parse_args():
    ...
    if args.doResponsePlots:
        ...
        JetFolderDirs = [
            "JetResponse/slimmedJets/JEC",
            "JetResponse/slimmedJets/noJEC",
            "JetResponse/slimmedJetsPuppi/JEC",
            "JetResponse/slimmedJetsPuppi/noJEC",
        ]
        ...
        for JetFolderDir in JetFolderDirs:
            plots += [
                (
                    JetFolderDir,
                    "reso_pt",
                    ["preso_eta05", "preso_eta13", "preso_eta21", "preso_eta25", "preso_eta30", "preso_eta50"],
                )
            ]
            plots += [
                (
...
def addPlots(plotter, folder, name, section, histograms, opts, Offset=False):
    ...
    if "PackedCandidates" in folder:
        ...
        for plot in plots:
            ...
            plot.setProperties(legendDx=0.05)
        plotter.append(
            "ParticleFlow/PackedCandidates/" + section,
            folders,
            PlotFolder(*plots, loopSubFolders=False, page="PackedCandidates", section=section),
        )
...
def main():
    ...
    plot_opts = {"reso_pt": styledict_resolution, "reso_pt_rms": styledict_resolution, "response_pt": styledict_response}
    for iptbin in range(len(ptbins) - 1):
