...
#singularity exec --env PYTHONPATH=$PYTHONPATH -B /data -B /scratch-persistent $IMG tfds build hep_tfds/heptfds/cms_pf/singleele --data_dir $DATA_DIR --manual_dir $MANUAL_DIR --overwrite &> logs/tfds_singleele.log &
#singularity exec --env PYTHONPATH=$PYTHONPATH -B /data -B /scratch-persistent $IMG tfds build hep_tfds/heptfds/cms_pf/singlegamma --data_dir $DATA_DIR --manual_dir $MANUAL_DIR --overwrite &> logs/tfds_singlegamma.log &
wait
