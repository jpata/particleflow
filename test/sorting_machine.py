...
for path_nr in range(3,13):
    path = "/home/aadi/praktika/particleflow/test/data/bin_{}k/processed".format(path_nr)
    try:
        os.makedirs(path)
    except OSError:
        print("Creation of the directory %s failed. Terminating Program!" % path)
        quit()
    else:
...
data_items = torch.load('50k_data.pkl')
#File numbering init
...
for j in range(0,len(data_items)):
    ...
    if i in range(3000,4000):
        torch.save(data_items[j], "data/bin_3k/processed/data_{}.pt".format(k3))
        k3 = k3 +1

    #bin 4k-5k
    if i in range(4000,5000):
        torch.save(data_items[j], "data/bin_4k/processed/data_{}.pt".format(k4))
        k4= k4 +1

    #bin 5k-6k
    if i in range(5000,6000):
        torch.save(data_items[j], "data/bin_5k/processed/data_{}.pt".format(k5))
        k5 = k5 +1

    #bin 6k-7k
    if i in range(6000,7000):
        torch.save(data_items[j], "data/bin_6k/processed/data_{}.pt".format(k6))
        k6 = k6 + 1

    #bin 7k-8k
    if i in range(7000,8000):
        torch.save(data_items[j], "data/bin_7k/processed/data_{}.pt".format(k7))
        k7 = k7 + 1
    #bin 8k-9k
    if i in range(8000,9000):
        torch.save(data_items[j], "data/bin_8k/processed/data_{}.pt".format(k8))
        k8 = k8 + 1

    #bin 9k-10k
    if i in range(9000,10000):
        torch.save(data_items[j], "data/bin_9k/processed/data_{}.pt".format(k9))
        k9 = k9 + 1

    #bin 10k-11k
    if i in range(10000,11000):
        torch.save(data_items[j], "data/bin_10k/processed/data_{}.pt".format(k10))
        k10 = k10 + 1

    #bin 11k-12k
    if i in range(11000,12000):
        torch.save(data_items[j], "data/bin_11k/processed/data_{}.pt".format(k11))
        k11 = k11 + 1

    #bin 12k-13k
    if i in range(12000,13000):
        torch.save(data_items[j], "data/bin_12k/processed/data_{}.pt".format(k12))
        k12 = k12 + 1                                                                            
