...
def visualize_event(file_path, iev=0):
    ...
    ax1.scatter(pos_trk_h[:, 0], pos_trk_h[:, 1], pos_trk_h[:, 2], 
                c=pn_trk_h, cmap=cmap, vmin=vmin, vmax=vmax, s=2, label='Tracker Hits', alpha=0.5)
    ax1.scatter(pos_calo_h[:, 0], pos_calo_h[:, 1], pos_calo_h[:, 2], 
                c=pn_calo_h, cmap=cmap, vmin=vmin, vmax=vmax, s=15, marker='s', label='Calo Hits', alpha=0.7)
    ...
    ax2.scatter(pos_trk_h[:, 0], pos_trk_h[:, 1], pos_trk_h[:, 2], 
                c=pn_trk_h, cmap=cmap, vmin=vmin, vmax=vmax, s=1, alpha=0.03)
    ax2.scatter(pos_calo_h[:, 0], pos_calo_h[:, 1], pos_calo_h[:, 2], 
                c=pn_calo_h, cmap=cmap, vmin=vmin, vmax=vmax, s=2, marker='s', alpha=0.03)

    ax2.scatter(pos_trk[:, 0], pos_trk[:, 1], pos_trk[:, 2], 
                c=pn_trk, cmap=cmap, vmin=vmin, vmax=vmax, s=40, marker='o', edgecolors='black', label='Tracks', alpha=1.0)
    ax2.scatter(pos_cl[:, 0], pos_cl[:, 1], pos_cl[:, 2], 
                c=pn_cl, cmap=cmap, vmin=vmin, vmax=vmax, s=60, marker='D', edgecolors='black', label='Clusters', alpha=1.0)
