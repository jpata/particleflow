...
def validate_inclusive(file_path, b_field):
    ...
    for iev in tqdm(range(num_events)):
        ...
        if np.any(mask_trk_h):
            ...
            for pn in np.unique(pns_trk_h):
                if pn == 0 or pn not in pn_to_truth: continue
    ...
    # Convert to numpy
    for k in track_results: track_results[k] = np.array(track_results[k])
    for k in calo_results: calo_results[k] = np.array(calo_results[k])
