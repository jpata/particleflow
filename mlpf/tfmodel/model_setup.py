...
def eval_model(model, dataset, config, outdir, jet_ptcut=5.0, jet_match_dr=0.1, verbose=False):
    ...
    for elem in tqdm(dataset, desc="Evaluating model"):
        ...
        for typ in ["gen", "cand", "pred"]:
            ...
            pt = awkward.from_iter([v[m] for v, m in zip(awkvals[typ]["pt"], valid)])
            eta = awkward.from_iter([v[m] for v, m in zip(awkvals[typ]["eta"], valid)])
            energy = awkward.from_iter([v[m] for v, m in zip(awkvals[typ]["energy"], valid)])
            phi = awkward.from_iter([v[m] for v, m in zip(phi, valid)])
