...
def unpack_target(y, num_output_classes, config):
    ...
    energy_bins_edges = tf.cast(tf.experimental.numpy.logspace(-1, 3.5, 1024), dtype=tf.float32)
    pt_bins_edges = tf.cast(tf.experimental.numpy.logspace(-1, 3.5, 1024), dtype=tf.float32)
