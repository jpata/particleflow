...
JET_LABEL_AK4 = "AK4 jets, $p_{T}$ > 20 GeV, $|\eta|$ < 2.5"
...
def load_nano(files):
    ret = []
    branches = [
        "PFMET_pt", "PuppiMET_pt", "Jet_rawFactor", "Jet_pt",
        "Jet_nConstituents", "Jet_chMultiplicity", "Jet_neMultiplicity",
        "Jet_nElectrons", "Jet_nMuons", "Jet_chEmEF", "Jet_chHEF",
        "Jet_hfEmEF", "Jet_hfHEF", "Jet_muEF", "Jet_neEmEF", "Jet_neHEF",
        "Electron_pt", "Muon_pt", "Jet_eta", "Jet_phi", "Jet_mass",
        "HLT_AK8PFJet500", "HLT_PFJet500", "HLT_VBF_DiPFJet125_45_Mjj1200",
        "PV_npvsGood", "run", "luminosityBlock", "event",
        "Flag_HBHENoiseFilter", "Flag_CSCTightHaloFilter", "Flag_hcalLaserEventFilter",
...
def get_jet_pt(data):
    ...
    low_pt_jets = (data["Jet_pt_corr"] > 5) & jetid
    ...
    # Return full mask including back-to-back
    full_mask = mask_2_jets.copy()
    # This is slightly tricky because back_to_back_mask is already indexed by mask_2_jets
    indices_2_jets = np.where(mask_2_jets)[0]
    final_indices = indices_2_jets[awkward.to_numpy(back_to_back_mask)]
    final_mask = np.zeros(len(mask_2_jets), dtype=bool)
    final_mask[final_indices] = True

    return final_mask, leading_jet_pt, subleading_jet_pt, dijet_mass
...
@click.option("--sample-label", default="di-jets", help="Label for the sample in plots")
def main(input_pf, input_mlpf, golden_json, jec_file, lumi_csv, output_dir, sample_label):
    ...
    # Plotting
    plot_configs = [
        ("nmu", lambda d: awkward.num(d["Muon_pt"], axis=1), np.linspace(0, 10, 11), "Number of muons"),
        ("nele", lambda d: awkward.num(d["Electron_pt"], axis=1), np.linspace(0, 20, 21), "Number of electrons"),
        ("njet", lambda d: awkward.num(d["Jet_pt_corr"][compute_jetid(d)], axis=1), np.linspace(0, 20, 21), "Number of jets"),
        ("jet_pt", lambda d: awkward.flatten(d["Jet_pt_corr"][compute_jetid(d)]), np.linspace(0, 2000, 201), plot_utils.labels["pt"]),
        ("jet_nConstituents", lambda d: awkward.flatten(d["Jet_nConstituents"]), np.linspace(0, 100, 101), "Jet constituents (Jet_nConstituents)"),
        ("jet_muEF", lambda d: awkward.flatten(d["Jet_muEF"]), np.linspace(0, 1, 101), "Jet muon frac (Jet_muEF)"),
        ("jet_chEmEF", lambda d: awkward.flatten(d["Jet_chEmEF"]), np.linspace(0, 1, 101), "Jet charged EM frac (Jet_chEmEF)"),
        ("jet_chHEF", lambda d: awkward.flatten(d["Jet_chHEF"]), np.linspace(0, 1, 101), "Jet charged hadronic frac (Jet_chHEF)"),
        ("jet_neEmEF", lambda d: awkward.flatten(d["Jet_neEmEF"]), np.linspace(0, 1, 101), "Jet neutral EM frac (Jet_neEmEF)"),
        ("jet_neHEF", lambda d: awkward.flatten(d["Jet_neHEF"]), np.linspace(0, 1, 101), "Jet neutral hadronic frac (Jet_neHEF)"),
        ("jet_hfEmEF", lambda d: awkward.flatten(d["Jet_hfEmEF"]), np.linspace(0, 2, 101), "Jet HF EM frac (hfEmEF)"),
        ("jet_hfHEF", lambda d: awkward.flatten(d["Jet_hfHEF"]), np.linspace(0, 1, 101), "Jet HF hadronic frac (Jet_hfHEF)"),
        ("jet_nElectrons", lambda d: awkward.flatten(d["Jet_nElectrons"]), np.linspace(0, 10, 11), "Jet num. electrons (nElectrons)"),
        ("jet_nMuons", lambda d: awkward.flatten(d["Jet_nMuons"]), np.linspace(0, 10, 11), "Jet num. muons (nMuon)"),
        ("jet_ch_mult", lambda d: awkward.flatten(d["Jet_chMultiplicity"]), np.linspace(0, 60, 61), "Jet charged multiplicity (chMultiplicity)"),
        ("jet_ne_mult", lambda d: awkward.flatten(d["Jet_neMultiplicity"]), np.linspace(0, 60, 61), "Jet neutral multiplicity (neMultiplicity)"),
        ("npvs", lambda d: d["PV_npvsGood"], np.linspace(0, 100, 101), "Number of good PVs")
    ]

    for name, func, bins, xlabel in plot_configs:
        plt.figure()
        ax = plt.axes()
        plt.hist(func(data_pf), bins=bins, histtype="step", label="PF", ls=PF_LINESTYLE, lw=2, color=PF_COLOR)
        plt.hist(func(data_mlpf), bins=bins, histtype="step", label="MLPF", ls=MLPF_LINESTYLE, lw=2, color=MLPF_COLOR)
        plt.yscale("log")
        mplhep.cms.label("", data=True, rlabel=lumi_text)
        ax.text(SAMPLE_LABEL_COORDS[0], SAMPLE_LABEL_COORDS[1], sample_label, va="top", transform=ax.transAxes, fontsize=SAMPLE_LABEL_FONTSIZE)
    ...
    event_label = sample_label + "
$\geq 2$ jets, $p_T>60$ GeV, $|\eta|<2.5$"

    # MET plot with ratio
    def plot_with_ratio(h_pf, h_mlpf, bins, xlabel, filename):
        f, (a0, a1) = plt.subplots(2, 1, gridspec_kw={"height_ratios": [3, 1]}, sharex=True)
        plt.sca(a0)
        mplhep.histplot(h_pf, histtype="step", label="PF-PUPPI", ls=PF_LINESTYLE, lw=2, color=PF_COLOR)
        mplhep.histplot(h_mlpf, histtype="step", label="MLPF-PUPPI", ls=MLPF_LINESTYLE, lw=2, color=MLPF_COLOR)
        plt.yscale("log")
        mplhep.cms.label("", data=True, rlabel=lumi_text)
        ...
        a0.legend(loc="best", fontsize=LEGEND_FONTSIZE)
        plt.ylabel("Counts")
        
        plt.sca(a1)
        # Ratio
        ratio = h_mlpf / h_pf
        # Simple error prop for ratio
        pf_counts = h_pf.counts()
        mlpf_counts = h_mlpf.counts()
        err = ratio * np.sqrt(np.where(pf_counts>0, 1/pf_counts, 0) + np.where(mlpf_counts>0, 1/mlpf_counts, 0))
        
        mplhep.histplot(h_pf/h_pf, color=PF_COLOR, ls=PF_LINESTYLE, lw=2)
        mplhep.histplot(ratio, color=MLPF_COLOR, ls=MLPF_LINESTYLE, lw=2)
        # band for PF error
        pf_err = 1.0 * np.sqrt(np.where(pf_counts>0, 1/pf_counts, 0))
        mplhep.histplot(h_pf/h_pf, yerr=pf_err, edgecolor=PF_COLOR, ls=PF_LINESTYLE, lw=2, histtype="band", facecolor=PF_COLOR, alpha=0.3)
    ...
    h_slj_pf = to_bh(slj_pt_pf, pt_bins)
    h_slj_mlpf = to_bh(slj_pt_mlpf, pt_bins)
    plot_with_ratio(h_slj_pf, h_slj_mlpf, pt_bins, "Subleading jet " + plot_utils.labels["pt"], "jetmet0_subleading_jet_pt.pdf")
    ...
    plot_with_ratio(h_asym_pf, h_asym_mlpf, asym_bins, r"Dijet asymmetry $(p^{1}_{T}-p^{2}_{T})/(p^{1}_{T}+p^{2}_{T})$", "jetmet0_dijet_asymmetry.pdf")
