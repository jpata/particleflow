...
import math
from mlpf.model.ElementBinner import ElementBinner
...
class LiteMLA(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, x_coords: torch.Tensor, mask: Optional[torch.Tensor] = None
    ) -> torch.Tensor:  # Output is now (B, N_eta, N_phi, F_out)
        """
        Args:
        ...
        if self.bin_pooling_mlp is not None:
            ...
            flattened_bin_features = masked_binned_x.reshape(B, N_eta, N_phi, self.binner.max_elems_per_bin * F_in_binned)
            ...
            aggregated_bin_features = aggregated_bin_features_flat.reshape(B, N_eta, N_phi, -1)
        else:
