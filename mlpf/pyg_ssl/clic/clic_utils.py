...
"""Based on https://github.com/jpata/particleflow/blob/main/mlpf/data_clic/postprocessing.py"""
...
def flatten_event(df_tr, df_cl, df_gen, df_pfs, pairs):
    ...
    if Xs_tracks.shape[1] > Xs_clusters.shape[-1]:
        Xs_clusters = np.pad(
            Xs_clusters, [(0, 0), (0, Xs_tracks.shape[1] - Xs_clusters.shape[-1])]
        )
    elif Xs_tracks.shape[1] < Xs_clusters.shape[-1]:
        Xs_clusters = np.pad(
            Xs_clusters, [(0, 0), (0, Xs_clusters.shape[-1] - Xs_tracks.shape[1])]
        )
...
def prepare_data_clic(fn):
    ...
    """
    Takes as input a parquet file that contains the CLIC raw information, and returns a list of PyG Data() objects.
    Each element of the list looks like this ~ Data(x=[#, 12], ygen=[#, 5], ygen_id=[#], ycand=[#, 5], ycand_id=[#])
