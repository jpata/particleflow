...
class IdeaEdmQqPf(tfds.core.GeneratorBasedBuilder):
    ...
    MANUAL_DOWNLOAD_INSTRUCTIONS = (
        "Point --manual_dir at a directory containing the IDEA parquet file."
    )
            ...
            features=tfds.features.FeaturesDict(
                {
                    ...
                    "ytarget": tfds.features.Tensor(
                        shape=(None, len(Y_FEATURES)), dtype=np.float32
                    ),
                    ...
                    "targetjets": tfds.features.Tensor(
    ...
    def _split_generators(self, dl_manager):
        files = find_idea_parquets(Path(dl_manager.manual_dir))
        return {
            name: self._generate_examples(refs)
            for name, refs in split_event_references(files).items()
        }
