...
class CmsPf(tfds.core.GeneratorBasedBuilder):
  ...
  def _split_generators(self, dl_manager: tfds.download.DownloadManager):
    ...
    return {
        'train': self._generate_examples(path / 'raw'),
        'test': self._generate_examples(path / 'val')
    }
  ...
  def prepare_data_cms(self, fn):
    ...
    for event in data:
        ...
        Xelem = append_fields(Xelem, "typ_idx", np.array([ELEM_LABELS_CMS.index(int(i)) for i in Xelem["typ"]], dtype=np.float32))
        ygen = append_fields(ygen, "typ_idx", np.array([CLASS_LABELS_CMS.index(abs(int(i))) for i in ygen["typ"]], dtype=np.float32))
        ycand = append_fields(ycand, "typ_idx", np.array([CLASS_LABELS_CMS.index(abs(int(i))) for i in ycand["typ"]], dtype=np.float32))
