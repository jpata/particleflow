...
- [ ] **Update Configuration Schema (`mlpf/conf.py`)**:
    - Add `particle_number` to the `ParticleFeatures` dataclass.
- [ ] **Generate Unique IDs (`mlpf/data/key4hep/postprocessing.py`)**:
...
- [ ] **Add OC Output Heads (`mlpf/model/mlpf.py`)**:
...
- [ ] **Prediction Unpacking (`mlpf/model/utils.py`)**:
...
- [ ] **Port OC Math (`mlpf/model/losses.py`)**:
...
## Summary Checklist
- [ ] `mlpf/conf.py`: Add `particle_number` to `ParticleFeatures`.
- [ ] `mlpf/data/key4hep/postprocessing.py`: Assign unique IDs in `assign_genparticles_to_obj_and_merge`.
- [ ] `mlpf/model/mlpf.py`: Add `oc_beta` and `oc_coords` heads.
- [ ] `mlpf/model/utils.py`: Update `unpack_predictions` and `unpack_target`.
- [ ] `mlpf/model/losses.py`: Integrate `calc_LV_Lbeta` into `mlpf_loss`.
